<?php 
  ...
  $kueri = mysqli_query($koneksi,"SELECT * FROM toko WHERE nama_toko='$akses'");
  $dkueri = mysqli_fetch_array($kueri);
  $idtok = $dkueri['id_toko'];

  $data1 = mysqli_query($koneksi,"SELECT * FROM kab_kota");
  $data2 = mysqli_query($koneksi,"SELECT * FROM toko");
  $data3 = mysqli_query($koneksi,"SELECT * FROM oleh_oleh WHERE id_toko='$idtok'");
  $data4 = mysqli_query($koneksi,"SELECT * FROM chat WHERE id_toko='$idtok'");

  $dx = mysqli_num_rows($data);
...
?>
    ...
    <div class="container-fluid mt-4">
			<div class="row justify-content-center">
				...
				<div class="card-deck col-auto-mb-3 m-2 ">
					<div class="card  my-3 shadow p-1 bg-primary text-light rounded" style="width: 12rem;">
					    <div class="card-body">
					      <h3 class="card-title text-center text-uppercase"><?php //echo $d4 ?></h3>
					    </div>
					    <div class="card-footer text-center p-1">
						   <p class="text-center m-0">CHAT</p>
						</div>
					    </a>
				    </div>
				</div> -->
